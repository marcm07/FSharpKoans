    ...
    *)
    ...
    let ``01 Finding the length of a list, the hard way`` () =
        let length (xs : 'a list) : int =
            let rec list_length listitem acc = // write a function to find the length of a list
                match listitem with
                | [] -> acc
    ...
    let ``03 Reversing a list, the hard way`` () =
        let rev (xs : 'a list) : 'a list =
            let rec reverse list acc = // write a function to reverse a list here.
                match list with
                | [] -> []
                | [x] -> x::acc
    ...
    let ``05 Fixed-function mapping, the hard way (part 1).`` () =
        let map (xs : int list) : int list =
            let rec add1 acc list =  // write a function which adds 1 to each element
                match list with
                | [] -> []
                | [x] -> x+1::acc
                | head::tail -> add1 (head+1::acc) tail 
            List.rev(add1 [] xs)
    ...
    let ``06 Fixed-function mapping, the hard way (part 2).`` () =
        let map (xs : int list) : int list =
            let rec double acc list =  // write a function which doubles each element
                match list with
                | [] -> []
                | [x] -> x*2::acc
                | head::tail -> double (head*2::acc) tail 
            List.rev(double [] xs) 
   ...
   *)
    ...
    let ``07 Specified-function mapping, the hard way`` () =
        let map (f : 'a -> 'b) (xs : 'a list) : 'b list =
            let rec apply acc list f = // write a map which applies f to each element
                match list with
                | [] -> []
                | [x] -> f(x)::acc
                | head::tail -> apply (f(head)::acc) tail f
            List.rev(apply [] xs f)
    ...
    let ``09 Specified-function filtering, the hard way`` () =
        let filter (f : 'a -> bool) (xs : 'a list) : 'a list =
            let rec filterlist acc f list=  // write a function which filters based on the specified criteria
                match list with
                |x::xs -> 
                    match f x with
                    |true -> filterlist (x::acc) f xs
                    |false -> filterlist acc f xs
                | [] -> acc
            List.rev(filterlist [] f xs) 
...
// ... you can make a function to do that, right? ^_^.
    ...
    let ``11 Fixed-function filtering, the hard way`` () =
        let filter (xs : int list) : int list =
            let rec filterlist acc list=  // write a function to filter for odd elements only.
                match list with
                |x::xs -> 
                    match x%2 with
                    |0 -> filterlist acc xs
                    |_ -> filterlist (x::acc) xs
                | [] -> acc 
            List.rev(filterlist [] xs )
...
*)
    ...
    let ``12 A fold which sums a list`` () =
        let fold initialState xs =
            let rec sum acc list = // write a function to do what's described above
                match list with
                | [] -> acc
    ...
    let ``13 A fold which multiplies a list`` () =
        let fold initialState xs =
            let rec multiply acc list = // write a function to multiply the elements of a list
                match list with
                |[] -> acc
    ...
    let ``14 Folding, the hard way`` () =
        let fold (f : 'a -> 'b -> 'a) (initialState : 'a) (xs : 'b list) : 'a =
            let rec do_fold acc list f =  // write a function to do a fold.
               match list with
               |[] -> acc
               |head::tail ->  do_fold (f acc head) tail f  
            do_fold initialState xs f
        fold (+) 0 [1;2;3;4] |> should equal 10
    ...
    let ``16 exists: finding whether any matching item exists`` () =
        let exists (f : 'a -> bool) (xs : 'a list) : bool =
            let rec check f list = // Does this: https://msdn.microsoft.com/en-us/library/ee370309.aspx
                match list with
                | [] -> false
                | x::xs -> 
                    match f x with
                    | true -> true
                    | false -> check f xs
            check f xs // Does this: https://msdn.microsoft.com/visualfsharpdocs/conceptual/list.exists%5b%27t%5d-function-%5bfsharp%5d
    ...
    let ``17 partition: splitting a list based on a criterion`` () =
        let partition (f : 'a -> bool) (xs : 'a list) : ('a list) * ('a list) =
            let rec split list1 list2 xs f = // Does this: https://msdn.microsoft.com/en-us/library/ee353782.aspx
                match xs with
                | [] -> List.rev(list1), List.rev(list2)
                | x::xs -> 
                    match f x with
                    |false -> split list1 (x::list2) xs f
                    |true -> split (x::list1) list2 xs f
            split [] [] xs f  // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.partition%5b't%5d-function-%5bfsharp%5d
    ...
    let ``18 init: creating a list based on a size and a function`` () =
        let init (n : int) (f : int -> 'a) : 'a list =
            let rec create acc size f list = // Does this: https://msdn.microsoft.com/en-us/library/ee370497.aspx
                match acc < size with
                | false -> list
                | true -> create (acc+1) size f (f(acc)::list)
            List.rev(create 0 n f []) // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.init%5b't%5d-function-%5bfsharp%5d
    ...
    let ``19 tryFind: find the first matching element, if any`` () =
        let tryFind (p : 'a -> bool) (xs : 'a list) : 'a option =
            let rec find f list = // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.tryfind%5b't%5d-function-%5bfsharp%5d
                match list with
                |[] -> None
                | x::xs ->
                    match f x with
    ...
    let ``20 tryPick: find the first matching element, if any, and transform it`` () =
        let tryPick (p : 'a -> 'b option) (xs : 'a list) : 'b option =
            let rec find f list = // Does this: https://msdn.microsoft.com/en-us/library/ee353814.aspx
                match list with
                |[] -> None
                |x::xs ->
                    match f x with
                    | Some x -> Some x
                    | None -> find f xs
            find p xs  // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.trypick%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    *)
    ...
    let ``21 choose: find all matching elements, and transform them`` () =
        ...
        let choose (p : 'a -> 'b option) (xs : 'a list) : 'b list =
            let rec find f list acc =  // Does this: https://msdn.microsoft.com/en-us/library/ee353456.aspx
                match list with 
                | [] -> acc
                | x::xs -> 
                    match f x with 
                    |Some x -> find f xs (x::acc)
                    |None -> find f xs acc
            List.rev(find p xs [])   // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.choose%5b't,'u%5d-function-%5bfsharp%5d
        let f x =
            match x<=45 with
            | true -> Some(x*2)
            | _ -> None
    ...
    let ``22 mapi: like map, but passes along an item index as well`` () =
        let mapi (f : int -> 'a -> 'b) (xs : 'a list) : 'b list =
            let rec map f index list acc = // Does this: https://msdn.microsoft.com/en-us/library/ee353425.aspx
                match list with
                |[] -> acc
                |head::tail -> map f (index+1) tail ((f index head)::acc)
            List.rev(map f 0 xs []) // Does this: https://msdn.microsoft.com/en-us/visualfsharpdocs/conceptual/list.mapi%5b't,'u%5d-function-%5bfsharp%5d
